...
module Repositories.ProductGR (getProductsByShop, getProductsByOrderId, getProductsWithOrdersId, getProductsByOrder) where
...
getProductsByOrder ord =
  let prd = getProductsByOrderId $ orderId ord
   in tell ["getProductsByOrder start"] >> prd >>= \res -> tell ["getProductsByOrder end"] >> return res

getProductsByOrderId :: Int -> App [Product]
getProductsByOrderId sId =
  tell ["getProductsByOrderId begin"]
    >> getList @ProductOrder
    >>= \out ->
...
getProductsWithOrdersId =
  tell ["getProductsWithOrdersId begin"]
    ...
    >>= ( mapM
            ...
            Control.Monad.>=> (\res -> tell ["getProductsWithOrdersId end"] >> return res)
        )